...
class gamelogic {
    ...
    public gamelogic() {
        for (int i = 0; i < cardvalues.colors[0,0].Length; i++) {for (int j = 0; j < 2; j++) {for (int x = j; x < cardvalues.numbers[0,0].Length; x++) {deck.Add(new card()); string[] newcolors = new string[] {cardvalues.colors[0,i], ""}; string[] newnumbers = {cardvalues.numbers[0, j], ""}; deck[deck.Count - 1].addcolor(newcolors, newnumbers); Console.WriteLine($"{newcolors[0]}, {newnumbers[1]}");}}}
    }
    
}

public class main { public static void Main(string[] args) { gamelogic game = new gamelogic(); Console.WriteLine($"reeee"); } }