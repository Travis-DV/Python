...
class gamelogic
{
	...
	public gamelogic()
	{
		for (int i = 0; i < cardvalues.colors[0, 0].Length; i++) { for (int j = 0; j < 2; j++) { for (int x = j; x < 12; x++) { deck.Add(new card()); string[] newcolors = new string[]{cardvalues.colors[0, i], ""}; string[] newnumbers = {cardvalues.numbers[0, x], ""}; Console.WriteLine($"{newcolors[0]}, {newnumbers[0]}"); deck[deck.Count - 1].addcolor(newcolors, newnumbers); } } }
	}
